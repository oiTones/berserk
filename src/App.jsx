...
export default function BerserkApp(){
  ...
  const handleAuth = (e) => {
    ...
    if (authMode === "signup") {
      ...
      // Simulação de login aceitando qualquer credencial válida localmente
      setUser({ name: authForm.username.trim().toUpperCase() });
    }
  ...
  // CÁLCULO INTELIGENTE MELHORADO (Tenta API, se falhar usa IA Analítica Local Baseada em Tokens)
  const calcWithAI = async () => {
    ...
    try {
      ...
      // INTERPRETADOR LOCAL CASO A API FALHE (Evita que o app quebre ou fique travado)
      let found = null;
      ...
      if (found) {
        ...
        // Fallback genérico inteligente se for um termo desconhecido
        setDraft(prev => ({
          ...prev,
  ...
  if (!loaded) {
    return <div className="flex items-center justify-center h-screen text-zinc-500 text-sm bg-zinc-950">Carregando Protocolos...</div>;
  }

  // RENDERIZAÇÃO DA TELA DE LOGIN / SIGNUP CASO NÃO ESTEJA LOGADO
  if (!user) {
    return (
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/80 p-6 rounded-2xl shadow-xl space-y-6 relative">
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full text-sm font-mono bg-zinc-950 border border-zinc-800 rounded-xl pl-10 pr-4 py-3 text-zinc-200 placeholder-zinc-700 focus:border-red-700 focus:outline-none transition-colors"
                  />
                </div>
              </div>
              ...
              <button type="submit" className="w-full bg-red-700 hover:bg-red-600 text-white rounded-xl py-3 text-sm font-black tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-red-950/40 active:scale-[0.98] transition-all uppercase">
                <LogIn size={15} />
                {authMode === "login" ? "Inicializar Sistema" : "Registrar Nova Diretriz"}
              </button>
            </form>
            ...
            <div className="text-center pt-2 border-t border-zinc-800/40">
              <button 
                ...
                className="text-xs text-zinc-500 hover:text-zinc-300 font-mono tracking-wide transition-colors"
              >
  ...
  return (
    <div className="max-w-md mx-auto bg-zinc-950 min-h-screen md:min-h-[750px] shadow-2xl overflow-hidden flex flex-col border-x border-zinc-900 text-zinc-100 selection:bg-red-500/20 selection:text-red-400" style={{fontFamily:"system-ui, -apple-system, sans-serif"}}>
      ...
      {/* Header com Nome do Usuário Logado e Logout */}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {view === "hoje" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-zinc-900/30 p-2 rounded-xl border border-zinc-900">
              <button onClick={() => shiftDate(-1)} className="p-2 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-red-500 active:scale-95 transition-all">
                <ChevronLeft size={16} />
              </button>
              <div className="text-center">
                <div className="text-sm font-bold text-zinc-200 tracking-wide uppercase">
                  {WEEKDAYS[wd]} {isToday && <span className="text-red-500 font-black animate-pulse">●</span>}
                </div>
            ...
            <div className="space-y-2">
              {tasks.length === 0 && (
                <p className="text-center text-xs text-zinc-600 py-12 font-medium tracking-wide">NENHUMA DIRETRIZ DEFINIDA PARA ESTE DIA.</p>
              )}
              {tasks.map(task => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all border ${checked ? "bg-red-950/10 border-red-900/40 shadow-inner shadow-red-950/30" : "bg-zinc-900 border-zinc-800/80 hover:border-zinc-700"}`}
                  >
                    <div className={`flex-shrink-0 w-5 h-5 rounded-md flex items-center justify-center border transition-all ${checked ? "bg-red-600 border-red-600 shadow-md shadow-red-600/20" : "border-zinc-700 bg-zinc-950"}`}>
        ...
        {view === "dieta" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-zinc-900/30 p-2 rounded-xl border border-zinc-900">
              <button onClick={() => shiftDate(-1)} className="p-2 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400 active:scale-95">
                <ChevronLeft size={16} />
              </button>
                ...
                {[["kcal","Kcal"],["protein","Prot(g)"],["carb","Carb(g)"],["fat","Gord(g)"]].map(([key,label])=>(
                  <div key={key}>
                    ...
                    <input type="number" value={targets[key]} onChange={e=>updateTarget(key, e.target.value)}
                      className="w-full text-xs font-mono bg-zinc-950 border border-zinc-800 rounded px-1.5 py-1.5 text-zinc-200 mt-1 focus:border-red-900 focus:outline-none transition-colors" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-zinc-900 border border-dashed border-zinc-800 rounded-xl p-4">
              ...
              <div className="flex gap-2 mb-2">
                <input type="text" placeholder="Alimento (ex: arroz branco)" value={draft.food}
                  ...
                  className="flex-1 min-w-0 text-sm bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-zinc-200 placeholder-zinc-600 focus:border-red-900 focus:outline-none" />
                <input type="number" placeholder="g" value={draft.grams}
                  onChange={e=>setDraft({...draft, grams:e.target.value})}
                  className="w-16 text-sm font-mono bg-zinc-950 border border-zinc-800 rounded-lg px-2 py-2 text-zinc-200 placeholder-zinc-600 text-center focus:border-red-900 focus:outline-none" />
              </div>
              <button onClick={calcWithAI} disabled={calculating}
                className="w-full bg-red-700 hover:bg-red-600 disabled:opacity-40 text-white rounded-lg py-2 text-sm font-bold flex items-center justify-center gap-1.5 active:scale-95 mb-3 shadow-md shadow-red-950/20 transition-all">
                ...
                {[["kcal","Kcal"],["protein","Prot"],["carb","Carb"],["fat","Gord"]].map(([key,label])=>(
                  <div key={key}>
                    ...
                    <input type="number" value={draft[key]} onChange={e=>setDraft({...draft,[key]:e.target.value})}
                      className="w-full text-xs font-mono bg-zinc-950 border border-zinc-800 rounded px-1.5 py-1.5 text-zinc-200 mt-1 focus:border-red-900 focus:outline-none" />
                  </div>
                ))}
              </div>
              <input type="text" placeholder="Vitaminas/minerais (opcional)" value={draft.vitamins}
                ...
                className="w-full text-xs bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-zinc-200 placeholder-zinc-600 mb-3 focus:border-red-900 focus:outline-none" />

              <button onClick={addMeal} className="w-full bg-zinc-200 hover:bg-zinc-100 text-zinc-950 rounded-lg py-2 text-sm font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-all">
            ...
            <div className="flex gap-1.5 mb-2 overflow-x-auto pb-1.5 scrollbar-none">
              {WD_SHORT.map((label, idx) => (
                <button key={idx} onClick={() => setEditWeekday(idx)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider flex-shrink-0 transition-all ${editWeekday===idx ? "bg-red-700 text-white shadow-md shadow-red-950/30" : "bg-zinc-900 text-zinc-500 border border-zinc-800 hover:text-zinc-300"}`}>
                  {label.toUpperCase()}
                </button>
              ))}
            </div>
              ...
              {(template[editWeekday]||[]).map(task => (
                <div key={task.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-2.5 flex items-center gap-2">
                  <input type="time" value={task.time} onChange={e => updateTaskField(task.id, "time", e.target.value)}
                    className="text-xs font-mono bg-zinc-950 border border-zinc-800 rounded p-1.5 w-18 text-zinc-200 focus:border-red-900 focus:outline-none" />
                  <input type="text" value={task.title} onChange={e => updateTaskField(task.id, "title", e.target.value)}
                    className="text-sm bg-zinc-950 border border-zinc-800 rounded p-1.5 flex-1 min-w-0 text-zinc-200 focus:border-red-900 focus:outline-none" />
                  <input type="number" value={task.points} onChange={e => updateTaskField(task.id, "points", e.target.value)}
                    className="text-xs font-mono bg-zinc-950 border border-zinc-800 rounded p-1.5 w-12 text-zinc-200 text-center focus:border-red-900 focus:outline-none" />
            ...
            <div className="bg-zinc-900 rounded-xl p-3 border border-dashed border-zinc-800">
              ...
              <div className="flex gap-2 mb-2">
                <input type="time" value={newTask.time} onChange={e=>setNewTask({...newTask, time:e.target.value})}
                  className="text-xs font-mono bg-zinc-950 border border-zinc-800 rounded p-1.5 w-18 text-zinc-200 focus:border-red-900 focus:outline-none" />
                <input type="text" placeholder="Nova tarefa" value={newTask.title} onChange={e=>setNewTask({...newTask, title:e.target.value})}
                  className="text-sm bg-zinc-950 border border-zinc-800 rounded p-1.5 flex-1 min-w-0 text-zinc-200 placeholder-zinc-700 focus:border-red-900 focus:outline-none" />
                <input type="number" placeholder="pts" value={newTask.points} onChange={e=>setNewTask({...newTask, points:e.target.value})}
                  className="text-xs font-mono bg-zinc-950 border border-zinc-800 rounded p-1.5 w-12 text-zinc-200 text-center placeholder-zinc-700 focus:border-red-900 focus:outline-none" />
              </div>
              <button onClick={addTaskToWeekday} className="w-full bg-red-700 hover:bg-red-600 text-white rounded-lg py-2 text-xs font-bold tracking-wider uppercase shadow-md shadow-red-950/20 transition-all">
                Injetar Diretriz
              </button>
            </div>
          </div>
        ...
        {view === "trofeus" && (
          <div className="space-y-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 shadow-lg text-center relative overflow-hidden">
              ...
              <div className="flex items-center justify-center gap-1.5 text-orange-500 mt-4 bg-black/40 w-fit mx-auto px-4 py-1.5 rounded-full border border-zinc-900">
                <Flame size={14} className="fill-orange-500/10" />
                <span className="text-xs font-bold">{streak()} Dias Inictos</span>
              </div>
              {nextTrophy && (
                <p className="text-[11px] text-zinc-500 mt-4 font-medium tracking-wide">Faltam <span className="font-mono font-bold text-zinc-300">{nextTrophy.threshold - totalPoints}</span> PTS para alcançar a patente {nextTrophy.label.toUpperCase()}</p>
              )}
            </div>
        ...
        {[
          ...
          return (
            <button key={tab.id} onClick={() => setView(tab.id)}
              className={`flex-1 flex flex-col items-center gap-1 py-3 transition-colors ${active ? "text-red-500 bg-red-950/5" : "text-zinc-600 hover:text-zinc-400"}`}>
